...
def test_allows_docker_inspect(monkeypatch):
    monkeypatch.setattr(bender, "run_argv", lambda argv, timeout=None: (0, "running", ""))
...
def test_allows_systemctl_status(monkeypatch):
    monkeypatch.setattr(bender, "run_argv", lambda argv, timeout=None: (3, "", ""))
    monkeypatch.setattr(bender, "_log_step", lambda *a, **k: None)
...
def test_denies_double_ampersand_compound_of_allowlisted_commands(monkeypatch):
    # Deliberate change: one command per call, matching the prompt and tool schema.
    monkeypatch.setattr(bender, "run_argv", lambda argv, timeout=None: (0, "", ""))
    monkeypatch.setattr(bender, "_log_step", lambda *a, **k: None)
    with pytest.raises(bender.DiagnosticNotAllowed, match="shell control operator"):
        bender.run_diagnostic("docker ps && docker logs CASA_GLUETON --tail 10")
...
def test_allows_docker_inspect_network_ip(monkeypatch):
    monkeypatch.setattr(bender, "run_argv", lambda argv, timeout=None: (0, "172.20.0.5", ""))
    monkeypatch.setattr(bender, "_log_step", lambda *a, **k: None)
    bender.run_diagnostic(
...
def test_denies_each_shell_control_operator(monkeypatch, operator):
    monkeypatch.setattr(bender, "run_argv", lambda *a, **k: pytest.fail("must not execute"))
    with pytest.raises(bender.DiagnosticNotAllowed, match="shell control operator") as exc:
        bender.run_diagnostic(f"docker ps {operator} docker ps")
    assert repr("newline" if operator == "\n" else operator) in str(exc.value)


def test_literal_shell_expansion_argument_and_timeout_are_preserved(monkeypatch):
    calls = []
    logged = []
    command = 'docker ps --filter "name=$HOME *"'
    result = (bender.RUN_ARGV_TIMEOUT_EXIT, "", "timed out")

    def run_argv(argv, timeout=None):
        calls.append((argv, timeout))
        return result

    monkeypatch.setattr(bender, "run_argv", run_argv)
    monkeypatch.setattr(bender, "_log_step", lambda *args: logged.append(args))
    assert bender.run_diagnostic(command) == result
    assert calls == [
        (["docker", "ps", "--filter", "name=$HOME *"], bender.COMMAND_TIMEOUT_SECONDS)
    ]
    assert logged == [("diagnostic", {"n": None, "command": command}, *result)]


@pytest.mark.parametrize("command", ["", "   "])
def test_denies_empty_argv(monkeypatch, command):
    monkeypatch.setattr(bender, "run_argv", lambda *a, **k: pytest.fail("must not execute"))
