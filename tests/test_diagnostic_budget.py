...
def test_executes_and_increments_under_budget(monkeypatch):
    monkeypatch.setattr(farnsworth, "_run_diagnostic_tool", lambda cmd, ev: f"ran: {cmd}")
...
def test_rejects_without_executing_once_budget_exhausted(monkeypatch):
    called = []
    monkeypatch.setattr(farnsworth, "_run_diagnostic_tool", lambda cmd, ev: called.append(cmd))
...
def test_multiple_calls_in_one_turn_share_the_same_budget(monkeypatch):
    ...
    monkeypatch.setattr(farnsworth, "_run_diagnostic_tool", lambda cmd, ev: "ok")
    calls_made = 0
    executed = 0
